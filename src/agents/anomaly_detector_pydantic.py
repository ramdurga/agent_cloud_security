...
class AnomalyDetectionAgent:
    ...
    def generate_recommendations(self, threat_level: ThreatLevel, event_type: EventType, event: Any = None) -> List[Tuple[int, str]]:
        ...
        if threat_level == ThreatLevel.HIGH:
            ...
            recommendations.append((2, "🔍 Check system logs for related activities"))
            recommendations.append((2, "🚨 Prepare incident response team for potential escalation"))
        
        # Priority 3: MEDIUM priority actions
        if threat_level == ThreatLevel.MEDIUM:
            recommendations.append((3, "👀 Monitor activity closely for next hour"))
            recommendations.append((3, "📊 Review security baselines and thresholds"))
            recommendations.append((3, "📝 Update threat intelligence database"))
            
        # Priority 4: LOW priority actions
        if threat_level == ThreatLevel.LOW:
            recommendations.append((4, "📈 Continue standard monitoring"))
            recommendations.append((4, "📋 Log event for trend analysis"))
            
        # Event-specific actions (add to existing recommendations)
